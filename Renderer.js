...
export class Renderer extends BaseRenderer {
  ...
  render(scene, camera) {
    ...
    this.device.queue.writeBuffer(
      lightUniformBuffer,
      16,
      new Float32Array([lightComponent.shininess])
    );
  ...
  renderNode(node, modelMatrix = mat4.create()) {
    ...
    const normalMatrix = this.mat3tomat4(
      mat3.normalFromMat4(mat3.create(), modelMatrix)
    );
  ...
  renderPrimitive(primitive) {
    const { materialUniformBuffer, materialBindGroup } = this.prepareMaterial(
      primitive.material
    );
    this.device.queue.writeBuffer(
